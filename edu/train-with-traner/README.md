...
関係演算子は 2 つの変数の大小関係を，等値演算子は 2 つの変数が同じ値かそうでないかを見ることができ，論理演算子はそれらの結果を合成することができます
...
ここまででデジタルの入出力ができたので，入力を読み取ってデータを加工し，出力に反映させるということをやってみましょう  
具体的にはスイッチが “押された瞬間” を検出してみます

では，まずどうしたら押された瞬間を検出できるかを考えてみましょう
...
ここでは「関数」というものを作っていきます．関数とはなにかから簡単に説明しましょう  
関数とは，ある決まった一連の処理をまとめたものです．今まで使ってきた `digitalWrite()` や `digitalRead()` など，後ろに丸括弧がつくものはすべて関数です

関数を使うとき（呼ぶ．などといいます），データを渡して呼ぶときがありますが，そのとき渡す値は「引数」（ひきすう）と呼ばれます．関数の機能や実装に応じて，任意の個数渡すことができます  
また，関数からデータが戻ってくることがあります．何かしらの計算をする関数を思い浮かべるとわかりやすいかもしれません．戻ってくるデータのことを「返り値」・「戻り値」などと呼びます  
これら引数や戻り値は一般に変数なので，「型」があります．型とは，簡単に言えばそのデータがどのような種類のものかを表すもので，文字（`char`），整数（`byte`, `short`, `int`, `long`），小数（`float`, `double`）などがあります

では，実際に関数を作ってみましょう．．．と行きたいところですがもう既に作っています．`void main()` や `void loop()` も関数なのです

ですがそれでは面白くないので，オリジナルの関数を作ってみましょう．書き方は以下の通り
...
一般化して書いてるのでちょっとわかりにくいですね．一例を示します  
とりあえず整数の値を 2 つ渡して，その和を計算させる関数を考えてみましょう
...
二種類の書き方で書いてみました．どちらも有効であり，同じ動きをします  
上は最もシンプルな書き方で，下は最も丁寧な書き方です

引数と返り値の型はどちらも `int` 型で，これは整数型です．なので整数の値しか扱うことができません  
というわけで練習問題．これを浮動小数点（小数）に対応させてください．名前は `sum_f` とし，型は `float` 型でお願いします

簡単なので答えは乗せません．正しいか知りたい場合は実際に動かすか作者に聞いて下さい

これの場合引数と返り値がありましたが，どちらか，あるいは両方ともに存在しない場合があります．その場合，型として `void` を用います
...
関数の返り値となる値は，`return` で指定することができます． `return answer;` とすれば，変数 `answer` が返り値となります  
この際，関数の返り値の型と，実際に返す値の型は一致させておいてください  
`void` 型の関数の場合，`return;` とすれば OK です

また，関数の中で `return` が実行されると否応なしにその関数の処理が終了します

### 第六節｜シフトレジスタと 7 セグメント LED

Trainer には，砲弾型 LED の他に 7 セグメント LED もあります．というかほとんどそれ関連の回路が占めています  
7 セグメント LED は名前通り 7 つのセグメント状の LED があり，おまけに Digit Point と呼ばれるちいさな LED もあります．つまり合計 8 個の LED 素子で構成されています  
そして，それぞれのアノードないしカソード（これは製品によって異なる）が個別に，もう一方の端子が一つにまとめられたパッケージになっています．共通になっている端子がアノードの場合アノードコモンと呼ばれ，Trainer で使用しているのはこれです．カソード側が共通の場合はカソードコモンです

アノードコモンなので，直接マイコンの GPIO に抵抗を介して接続して駆動しようとすると負論理となり若干扱いにくくなります．また，貴重な GPIO が 8 本も専有され，ちょっと困ったことになります  
そこで，これらの問題点を解決するため，それぞれ IC を使用しています

【GPIO で直接駆動すると負論理になる問題】  
この問題の解決のため，トランジスタ（部品としてはトランジスタアレイ）を用いて駆動しています．こうすることで実質的に NOT 回路となり，正論理にできます  
ついでに GPIO に流れる電流を小さくでき，マイコンへの負担が小さくなります  
トランジスタアレイを使うことで入力抵抗を省くことができ，部品点数も減らせるため単体トランジスタではなくこちらを採用しています

【貴重な GPIO が 8 本も専有される問題】  
この問題の解決のため，シフトレジスタを使用しています  
これは，シリアルで送ったデータをパラレルで出力してくれる部品です．クロックを入力するたびに内部でデータが一つ隣へ移動していき，IC 内部に蓄積されます  
それをラッチクロックを入力することで並列して出力できます  
このようにすることで，データ量の分だけ転送に時間がかかることを許容すれば，3 本の GPIO だけで 8 * 任意の整数だけ出力を拡張できます

Trainer では，シフトレジスタの出力をトランジスタアレイに接続して 7 セグメント LED を駆動しています  
なので，シフトレジスタにデータを送れば，7 セグメント LED を制御できるわけです
...
- 関数：特定の処理をまとめたもの
- 引数：関数を呼ぶときに渡す値
- 返り値・戻り値：関数から渡される・戻って来る値
- 型：データの種類．文字，整数，小数などがある
