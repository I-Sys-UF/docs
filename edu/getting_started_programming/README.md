...
`loop` 関数では，`static uint8_t` 型で 4 つの変数を，`static int8_t` 型で 1 つの変数を宣言している．`uint8_t` , `int8_t` は良いとして，見慣れない `static` という修飾子がついているが，これを付けることにより，関数が実行されたあとも変数と値が保持され，再度同じ関数がコールされたときに引き続き使うことができるようになる  
